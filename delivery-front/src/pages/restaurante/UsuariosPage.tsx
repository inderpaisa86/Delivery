...
export function UsuariosPage() {
  ...
  const [username, setUsername] = useState(''); const [password, setPassword] = useState(''); const [foto, setFoto] = useState<string | null>(null); const [perfil, setPerfil] = useState('operario'); const [formError, setFormError] = useState('');
  ...
  const crear = useMutation({ mutationFn: () => { setFormError(''); return usuarioService.crear({ username, password, foto, perfil, restauranteId: restaurante!.id }); }, onSuccess: () => { qc.invalidateQueries({ queryKey: ['usuarios', restaurante?.id] }); toast('Creado', 'success'); closeForm(); }, onError: (e: Error) => { setFormError(e.message); toast(e.message, 'error'); } });
  const actualizar = useMutation({ mutationFn: () => { setFormError(''); return usuarioService.actualizar(editando!.id, { username, password, foto, perfil, restauranteId: restaurante!.id }); }, onSuccess: () => { qc.invalidateQueries({ queryKey: ['usuarios', restaurante?.id] }); toast('Actualizado', 'success'); closeForm(); }, onError: (e: Error) => { setFormError(e.message); toast(e.message, 'error'); } });
  ...
  const closeForm = () => { setShowForm(false); setEditando(null); setUsername(''); setPassword(''); setFoto(null); setPerfil('operario'); setFormError(''); };
  const openEdit = (u: UsuarioResponse) => { setEditando(u); setUsername(u.username); setPassword(''); setFoto(u.foto); setPerfil(u.perfil || 'operario'); setShowForm(true); };
    ...
    <div className="animate-fade-up">
          ...
          {usuarios.map((u) => (
            <div key={u.id} className="bg-white rounded-2xl border border-[var(--border)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] transition-all p-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0"><h3 className="font-bold">{u.username}</h3><p className="text-xs text-[var(--text-muted)]">{u.perfil === 'admin' ? '👑 Administrador' : '👤 Operario'}</p></div>
      ...
      <Modal open={showForm} onClose={closeForm} title={isEditing ? 'Editar Usuario' : 'Nuevo Usuario'}>
        <form onSubmit={(e) => { e.preventDefault(); if (!isEditing && password.length < 6) { setFormError('Contraseña mínimo 6 caracteres'); return; } isEditing ? actualizar.mutate() : crear.mutate(); }} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-[var(--text-secondary)] mb-1.5">Perfil</label>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setPerfil('operario')} className={`py-2.5 rounded-xl text-sm font-medium border-2 transition-all ${perfil === 'operario' ? 'border-[var(--accent)] bg-[var(--accent-light)] text-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-muted)]'}`}>👤 Operario</button>
              <button type="button" onClick={() => setPerfil('admin')} className={`py-2.5 rounded-xl text-sm font-medium border-2 transition-all ${perfil === 'admin' ? 'border-[var(--accent)] bg-[var(--accent-light)] text-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-muted)]'}`}>👑 Admin</button>
            </div>
          </div>
